    ...
    >
      <div className="flex flex-col">
        ...
        <div className="flex flex-row gap-3">
          {Array.isArray(props.colaborador.premios) &&
            props.colaborador.premios?.length > 0 && (
              <span className="text-sm text-green-500 font-bold text-">
                Ganhou&nbsp;{props.colaborador.premios?.length}&nbsp;prêmio(s)
              </span>
            )}
        </div>
        <div>
          <span className="mr-2 text-sm text-orange-200 font-bold">
            {trataColaborador(props.colaborador.situacao)}
          </span>
          <span className="text-sm text-zinc-200 font-bold">
            {getCompanyName(props.colaborador.empresa)}
          </span>
        </div>
      </div>
    </div>
