...
export default function useColaboradorPremio() {
  const [colaboradorPremio, setColaboradorPremio] =
    useState<Partial<ColaboradorPremio> | null>(null);
  ...
  async function excluirPremioPorId(id: number) {
    if (!id) return;
    await Backend.colaboradorPremios.excluirPorId(id);
  }
